...
1. `DESCRIPTION`: set `Version` to the release version, without the development
   suffix.
2. `CITATION.cff`: set `version` to the same, and set `date-released` to the day
   the release is published. The file names the release it is heading for and is
   never set to a release candidate, even while a candidate is the newest thing
   archived on Zenodo.
3. `NEWS.md`: the section for the release carries its version as its heading.
4. `cran-comments.md`: describe the submission and confirm the check results
   against a fresh run rather than an older one.
...
and `CITATION.cff`, ignoring any `-rcN` suffix, so a mismatch fails the tag.
